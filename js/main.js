document.addEventListener('DOMContentLoaded', () => {
    ...
    spaceshipCursor.style.display = 'none'; // Hide initially
    ...
    function createShockwave(x, y, color) {
      ...
      const animate = () => {
        ...
        if (shockwave.life > 0 && shockwave.radius < shockwave.maxRadius) {
          ...
          ctx.stroke();
                ...
                // Continue animation only if running
      if (animationRunning) {
      ...
      // Start animation (now controlled by toggle)
      // animate();
    }
    ...
    function animate(timestamp) {
      ...
      // Request next frame only if running
      if (animationRunning) {
          animationId = requestAnimationFrame(animate);
      }
    }
